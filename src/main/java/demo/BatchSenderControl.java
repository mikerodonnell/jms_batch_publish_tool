...
public class BatchSenderControl {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		final AbstractApplicationContext context = new ClassPathXmlApplicationContext(SPRING_CONTEXT_FILE_NAME);
		...
		try {
			int delaySeconds = getDelaySeconds(reader);
			...
			if (MainMenuChoice.DIRECTORY.equals(mainMenuChoice)) {
				...
				batchSender.send(messageDirectory, delaySeconds);
				...
				batchSender.send(template, inputs, delaySeconds);
			} else {
				messageCount = getMessageCount(reader);
				batchSender.send(messageCount, delaySeconds);
			}
		} finally {
			reader.close();
		}
	...
	private static int getDelaySeconds(final BufferedReader reader) throws IOException {
		...
		while (true) {
			...
			try {
				delaySeconds = Integer.parseInt(input);
